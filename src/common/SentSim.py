...
class SentEmbedding(object):
    ...
    def prepare(self,all_sents:List[List[str]]):
        all_sents = [sent for sent_list in all_sents for sent in sent_list]
        self.generate_wordvecs()
        sentences = [' '.join(list(jieba.cut(sent))) for sent in all_sents]
        weightpara = 1e-3
    ...
    def sents_embedding(self, sentences:List[str]):
        sentences = [' '.join(list(jieba.cut(st))) for st in sentences]
